...
//
//    private final Validator<Member> userValidator;
//
...
//
//    public UserServiceImpl(UserRepository userRepository, Validator<Member> userValidator, ModelMapper modelMapper, MailService mailService, MailAddressRepository mailAddressRepository, ApplicationEventPublisher applicationEventPublisher, AuthService authService, SettingService settingService) {
...
//        Member user = findUserById(userDTO.getId());
...
//        Member user = findUserById(userId);
//        return UserMapper.map(user);
//    }
//
//    @Override
...
//        Member user = userRepository.findByAuth0UserId(authId)
...
//        Member user = this.findUserById(userId);
...
//    }
//
//    private MailAddress sendVerificationMail(MailAddress mailAddress, Member user) {
...
//    }
//
//    private Member saveUser(Member user) {
...
//        Member user = mailAddress.getUser();
//        MailEntryDTO mailEntryDTO = new MailEntryDTO(user.getFirstName(), user.getFullLastName(), mailAddress.getMailAddress());
//        mailService.addMailAddressToMailingList(mailEntryDTO);
//        if (mailAddress.receivesNewsletter()) {
//            mailService.addMailAddressToNewsletter(mailEntryDTO);
